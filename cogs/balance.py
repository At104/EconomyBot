...
class Balance(commands.Cog):
    ...
    async def balance(self, ctx):
        embed = discord.Embed(
            title="Balance",
            description="A delve into your finances",
        ...
        income_amount = id_nums[str(yourId)][0]["income"]
        gold_amount = id_nums[str(yourId)][0]["gold"]
        
        embed.add_field(name="**Balance** :bank:", value="$" + str(balance_amount))
        embed.add_field(name="**Workers** :construction_worker:", value=str(workers_amount))
        embed.add_field(name="**Income** :money_with_wings:", value="$" + str(income_amount) + "/hr")
        embed.add_field(name="**Gold** :coin:", value=str(gold_amount))
